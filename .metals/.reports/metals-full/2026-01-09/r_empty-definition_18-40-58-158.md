error id: file://<WORKSPACE>/api-gateway/src/main/java/com/example/api_gateway/auth/UserContextGrpcClientInterceptor.java:_empty_/Authentication#getAuthorities#stream#map#
file://<WORKSPACE>/api-gateway/src/main/java/com/example/api_gateway/auth/UserContextGrpcClientInterceptor.java
empty definition using pc, found symbol in pc: _empty_/Authentication#getAuthorities#stream#map#
        ...
        return new ForwardingClientCall.SimpleForwardingClientCall<ReqT,RespT>(next.newCall(method, callOptions)) {
            ...
            public void start(Listener<RespT> respListener, Metadata headers) {
                if(auth != null && auth.isAuthenticated()) {
                    ...
                    headers.put(GrpcAuthConstants.USER_ROLES, auth.getAuthorities().stream().@@map(authority -> authority.getAuthority()).collect(Collectors.joining(",")));
                }
